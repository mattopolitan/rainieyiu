...
class Item_Info {
	...
	protected function get_shipment_info_schema() {
		...
		return array(
			'order_id'      => array(
				'error'  => __( 'Shipment "Order ID" is empty!', 'pr-shipping-shipany' ),
			),
			'prefix' 		=> array(
				'default' => 'ShipAny'
			),
			'description' 	=> array(
				...
				'validate' => function( $value ) {
					...
					if( empty( $value ) && $this->isCrossBorder ) {
						throw new Exception( __( 'Shipment "Description" is empty!', 'pr-shipping-shipany' ) );
					}
				},
			),
			'weight'     => array(
                'error'    => __( 'Order "Weight" is empty!', 'pr-shipping-shipany' ),
                'validate' => function( $weight ) use ($self) {
                    if ( ! is_numeric( $weight ) || $weight < 0 ) {
                        throw new Exception( __( 'The order "Weight" must be a positive number', 'pr-shipping-shipany' ) );
                    }
                },
			...
			'shipany_product' => array(
				'rename' 	=> 'product_code',
                'error'     => __( '"Product" is empty!', 'pr-shipping-shipany' ),
			),
			'duties' => array(
				'rename' 	=> 'incoterm',
				'default' 	=> 'Duties',
			...
			'items_value' => array(
				'error'  => __( 'Shipment "Value" is empty!', 'pr-shipping-shipany' ),
				'validate' => function( $value ) {
					if ( ! is_numeric( $value ) ) {
						throw new Exception( __( 'The order "value" must be a number', 'pr-shipping-shipany' ) );
					}
				},
			...
			'currency' => array(
				'error' => __( 'Shop "Currency" is empty!', 'pr-shipping-shipany' ),
			),
            'cod_value' => array(
                'default' => 0,
                'rename' => 'codValue',
	...
	protected function get_recipient_info_schema() {
		...
		return array(
			'name'      => array(
				'error'  => __( 'Recipient is empty!', 'pr-shipping-shipany' ),
			...
			'address_1' => array(
				'rename' => 'address1',
				'error' => __( 'Shipping "Address 1" is empty!', 'pr-shipping-shipany' ),
			),
			'address_2' => array(
				'rename' => 'address2',
				'default' => '',
			...
			'postcode'  => array(
				'rename' => 'postCode',
				'error' => __( 'Shipping "Postcode" is empty!', 'pr-shipping-shipany' ),
			),
			'district' => array(
				'default' => ''
			),
			...
			'country'   => array(
				'error' => __( 'Shipping "Country" is empty!', 'pr-shipping-shipany' ),
			),
			'company'     => array(
				'default' => '',
			), 
	...
	protected function get_shipper_info_schema() {
		...
		return array(
			'shipany_contact_name'      => array(
				'rename' => 'name',
				'error'  => __( '"Account Name" in settings is empty.', 'pr-shipping-shipany' ),
				'sanitize' => function( $name ) use ($self) {

                    if (empty($name)) {
                        throw new Exception(
                            __( '"Account Name" in settings is empty.', 'pr-shipping-shipany' )
                        );
                    }
			...
			'shipany_address_1' => array(
				'rename' => 'address1',
				'error' => __( 'Base "Address 1" is empty!', 'pr-shipping-shipany' ),
                'sanitize' => function( $name ) use ($self) {

                    if (empty($name)) {
                        throw new Exception(
                            __( 'Base "Address 1" is empty!', 'pr-shipping-shipany' )
                        );
                    }
			...
			'shipany_city'      => array(
				'rename' => 'city',
				'error' => __( 'Base "City" is empty!', 'pr-shipping-shipany' ),
			),
			...
			'shipany_postcode'  => array(
				'rename' => 'postCode',
				'error' => __( 'Base "Postcode" is empty!', 'pr-shipping-shipany' ),
			),
			'shipany_state'     => array(
				'rename' => 'state',
				'default' => '',
			...
			'shipany_country'   => array(
				'rename' => 'country',
				'error' => __( 'Base "Country" is empty!', 'pr-shipping-shipany' ),
			),
		);
	}
	...
	protected function get_content_item_info_schema()
	{
		...
		return array(
			...
			'product_id'  => array(
				'error' => __( 'Item "Product ID" is empty!', 'pr-shipping-shipany' ),
			),
			'sku'         => array(
				'error' => __( 'Item "Product SKU" is empty!', 'pr-shipping-shipany' ),
			),
			'item_value'       => array(
				'rename' => 'value',
				'default' => 0,
			...
			'qty'         => array(
				'validate' => function( $qty ) {
					...
					if( !is_numeric( $qty ) || $qty < 1 ){
						...
						throw new Exception(
							__( 'Item quantity must be more than 1', 'pr-shipping-shipany' )
						);
